import { getDetailPlayList, listPlayList } from '@/lib/actions/playlist.actions';
import { Autocomplete, AutocompleteItem, Button, Checkbox, Modal, ModalContent } from '@nextui-org/react';
...
import { VideoPlaylist, VideoPlayListInProcess, VideoWebapp } from '@/lib/model/videoPlaylist';
import { CourseLesson, CourseVideo } from '@prisma/client';
import { addCourseVideoMany, deleteCourseVideoMany } from '@/lib/actions/video.actions';
...
const ManageContent = ({
  ...
  const [videoInLesson, setVideoInLesson] = useState<{[x: string]: VideoPlaylist}>({})
  ...
  const addVideoToLesson = (video: VideoWebapp) => {
    ...
    setVideoInLesson(prev => {
      ...
      const contentName = video.video_library_onlinevideoplaylistmediaitemcontent ? video.video_library_onlinevideoplaylistmediaitemcontent.name : null
      return {
        ...prev,
        [`${video.id}`]: {
          name: video.name,
  ...
  const removeVideoToLesson = (video: VideoPlaylist | VideoWebapp) => {
    ...
    setVideoInLesson((prevVideo) => {
      ...
      console.log(cloneVideo[`${videoId}`]);
      if(cloneVideo[`${videoId}`].action === "inDB" || cloneVideo[`${videoId}`].action === "removeInDB"){
        console.log('in if');
        cloneVideo[`${videoId}`] = {
          ...cloneVideo[`${videoId}`],
          action: "removeInDB"
  ...
  const submitSaveVideoInLesson = async () => {
    ...
    // remove from DB
    const idListRemoveFromDB = Object.keys(videoInLesson).map((key) => videoInLesson[key]).filter(video => video.action === "removeInDB").map(video => video.id!)
    console.log("removeFromDB", idListRemoveFromDB);
    const responseDelete = await deleteCourseVideoMany(idListRemoveFromDB)
    console.log("responseDelete",responseDelete);

    // add to DB
    const addToDB = Object.keys(videoInLesson).map((key) => videoInLesson[key]).filter(video => video.action === "create")
      ...
      {() => (
        <div className="p-app h-full flex flex-col">
                      ...
                      >
                        <div>
                          <Checkbox
                            color="default"
                            classNames={{
                              wrapper: 'after:bg-default-foreground',
                              icon: 'before:bg-red-400 text-white'
                            }}
                            // defaultSelected={defaultSelectedVideo(video)}
                            isSelected={defaultSelectedVideo(video)}
                            onValueChange={(isSelected) => {
                              handleOnSelectVideo(video, isSelected)
                            }}
                          ></Checkbox>
                        </div>
              ...
              {Object.keys(videoInLesson).map((key, index) => {
                ...
                return (
                  <div
                    ...
                    className={`font-IBM-Thai-Looped ${action === "removeInDB" ? `hidden` : `flex`} items-center p-2 border-2 border-default-foreground rounded-lg`}
                  >
                    <div className={`flex-1`}>
          ...
          <div className={`flex justify-end mt-3`}>
            <Button onClick={submitSaveVideoInLesson} className={`bg-default-foreground text-base font-medium font-IBM-Thai text-primary-foreground`} >
              บันทึก
            </Button>
          </div>
        </div>
